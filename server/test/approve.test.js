...
describe('GET /userApproval/:id', () => {
  it('should approve a user successfully', async () => {
    const userId = 'existingUserId'; // Replace with an existing user ID in your database
  ...
  it('should return 400 if user is already approved', async () => {
    const userId = 'alreadyApprovedUserId'; // Replace with a user ID that is already approved in your database
    const response = await request(app).get(`/userApproval/${userId}`);
...
// Add more test blocks if needed for other routes or scenarios
